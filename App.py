...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'Guruvi' and ad_password == '12345':
                st.success("Welcome Guruvi")
                # Display Data
                ...
                counts = df['Predicted Field'].value_counts()
                fig = px.pie(names=counts.index, values=counts.values, title='Predicted Field according to the Skills')    
                st.plotly_chart(fig)
                ...
                counts = df['User Level'].value_counts()
                fig = px.pie(names=counts.index, values=counts.values, title='Predicted Field according to the Skills')  
                st.plotly_chart(fig)
